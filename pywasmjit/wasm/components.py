...
class Module(WASMComponent):
    ...
    def to_file(self, f):
        f.write(b'\x00asm')
        f.write(pack_u32(1))  # version, must be 1 for now
...
class TypeSection(Section):
    ...
    def get_binary_section(self, f):
        f.write(pack_vu32(len(self.functionsigs)))  # count
...
class ImportSection(Section):
    ...
    def get_binary_section(self, f):
        f.write(pack_vu32(len(self.imports)))  # count
...
class FunctionSection(Section):
    ...
    def get_binary_section(self, f):
        f.write(pack_vu32(len(self.indices)))
...
class MemorySection(Section):
    ...
    def get_binary_section(self, f):
        f.write(pack_vu32(len(self.entries)))
        for entrie in self.entries:
            ...
            if len(entrie) == 1:
                f.write(pack_vu1(0))
                f.write(pack_vu32(entrie[0]))  # initial, no max
            else:
                f.write(pack_vu1(1))
                f.write(pack_vu32(entrie[0]))  # initial
                f.write(pack_vu32(entrie[1]))  # maximum
...
class ExportSection(Section):
    ...
    def get_binary_section(self, f):
        f.write(pack_vu32(len(self.exports)))
...
class CodeSection(Section):
    ...
    def get_binary_section(self, f):
        f.write(pack_vu32(len(self.functiondefs)))
...
class DataSection(Section):
    ...
    def get_binary_section(self, f):
        f.write(pack_vu32(len(self.chunks)))
        for chunk in self.chunks:
            f.write(pack_vu32(chunk[0]))
            # ff.write(packvu32(chunk[1]))
            Instruction('i32.const', chunk[1]).to_file(f)  # todo: is this right?
            f.write(pack_vu32(len(chunk[2])))
...
class Import(WASMComponent):
    ...
    def to_file(self, f):
        f.write(pack_str(self.modname))
        f.write(pack_str(self.fieldname))
        if self.kind == 'function':
            f.write(b'\x00')
            f.write(pack_vu32(self.type))
        else:
...
class FunctionSig(WASMComponent):
    ...
    def to_file(self, f):
        ...
        f.write(pack_vu32(len(self.params)))  # params
        for paramtype in self.params:
            f.write(LANG_TYPES[paramtype])
        f.write(pack_vu1(len(self.returns)))  # returns
...
class FunctionDef(WASMComponent):
    ...
    def to_file(self, f):
        ...
        f3 = BytesIO()
        f3.write(pack_vu32(len(local_entries)))  # number of local-entries in this func
        for localentry in local_entries:
            f3.write(pack_vu32(localentry[0]))  # number of locals of this type
            f3.write(LANG_TYPES[localentry[1]])
        for instruction in self.instructions:
            instruction.to_file(f3, self.module)
        f3.write(b'\x0b')  # end
        body = f3.getvalue()
        f.write(pack_vu32(len(body)))  # number of bytes in body
        f.write(body)
...
_exportable_classes = WASMType, WASMComponent, Function, ImportedFunction
